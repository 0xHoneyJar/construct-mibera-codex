...
files. Three rewrite patterns:
  ...
  3. mibera.s3.amazonaws.com/traits/{file}
     → assets.0xhoneyjar.xyz/Mibera/traits/{file}
...
# Pattern 3: mibera S3 bucket (trait images)
PAT_MIBERA_S3 = re.compile(
    r"https://mibera\.s3\.amazonaws\.com/traits/"
)
...
def rewrite_line(line):
    ...
    mibera_s3_count = len(PAT_MIBERA_S3.findall(line))

    if s3_count or irys_count or mibera_s3_count:
        line = PAT_S3.sub(f"https://{CDN_HOST}/", line)
        line = PAT_IRYS.sub(f"https://{CDN_HOST}/reveal_phase8/images/", line)
        line = PAT_MIBERA_S3.sub(f"https://{CDN_HOST}/Mibera/traits/", line)

    return line, s3_count, irys_count, mibera_s3_count
...
def collect_old_urls(files):
    ...
    for fpath in files:
        try:
            ...
            continue
        for pat, url_list in [(PAT_S3, s3_urls), (PAT_IRYS, irys_urls), (PAT_MIBERA_S3, mibera_s3_urls)]:
            for match in pat.finditer(content):
                start = match.start()
                rest = content[start:]
                url_match = re.match(r'(https://[^\s"\')\]>]+)', rest)
                if url_match:
                    url_list.append(url_match.group(1))
    return s3_urls, irys_urls, mibera_s3_urls
...
def rewrite_old_to_new(old_url):
    ...
    url = PAT_MIBERA_S3.sub(f"https://{CDN_HOST}/Mibera/traits/", url)
    return url
...
def run_verify(n, files):
    ...
    s3_urls, irys_urls, mibera_s3_urls = collect_old_urls(files)
    print(f"  Found {len(s3_urls)} S3-direct, {len(irys_urls)} Irys, {len(mibera_s3_urls)} mibera-S3 URLs")

    all_ok = True
    for label, url_list in [("S3-direct", s3_urls), ("Irys", irys_urls), ("mibera-S3", mibera_s3_urls)]:
...
def main():
    ...
    for fpath in files:
        ...
        for line in content.splitlines(keepends=True):
            new_line, s3_c, irys_c, ms3_c = rewrite_line(line)
            new_lines.append(new_line)
            file_s3 += s3_c
            file_irys += irys_c
            file_mibera_s3 += ms3_c

        if file_s3 or file_irys or file_mibera_s3:
            total_s3 += file_s3
            total_irys += file_irys
            total_mibera_s3 += file_mibera_s3
            files_modified += 1
    ...
    print(f"    mibera-S3 (traits):      {total_mibera_s3:,} refs")
    print(f"    Total:                   {total_s3 + total_irys + total_mibera_s3:,} refs")
